...
uint8_t last_driver_data[SN74X165_LENGTH] = {0, 0};
uint8_t driver_data[SN74X165_LENGTH]      = {0, 0};
...
bool matrix_scan_custom(matrix_row_t current_matrix[]) {
    ...
    // pulsePinLow(SN74X165_LATCH_PIN);
    ...
    sn74x165_spi_receive((uint8_t*)driver_data);
    // spi_start(SN74X165_SPI_SLAVE_SELECT_PIN, SN74X165_SPI_LSBFIRST, 0, SN74X165_SPI_CLOCK_DIVISOR);
    // spi_receive((uint8_t*)driver_data, SN74X165_LENGTH);
    // spi_stop();
