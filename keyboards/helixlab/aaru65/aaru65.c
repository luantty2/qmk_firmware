...
enum via_fader_value {
    ...
    id_gelix_mode     = 11,
    id_gelix_delay    = 12,
    id_gelix_duration = 13,
};

// enum via_gelix_value {
//     id_gelix_mode     = 1,
//     id_gelix_delay    = 2,
//     id_gelix_duration = 3,
// };

// const uint8_t id_gelix_channel = 20;  //via command channel
...
void gelix_config_set_value(uint8_t *data) {
    // data = [ value_id, value_data ]
    uint8_t *value_id   = &(data[0]);
    uint8_t *value_data = &(data[1]);
    switch (*value_id) {
        case id_gelix_mode:
            gelix_config.mode = *value_data;
            break;
        case id_gelix_delay:
            gelix_config.delay = *value_data;
            break;
        case id_gelix_duration:
            gelix_config.duration = *value_data;
            break;
    }
}

void gelix_config_get_value(uint8_t *data) {
    // data = [ value_id, value_data ]
    uint8_t *value_id   = &(data[0]);
    uint8_t *value_data = &(data[1]);
    switch (*value_id) {
        case id_gelix_mode:
            value_data[0] = gelix_config.mode;
            break;
        case id_gelix_delay:
            value_data[0] = gelix_config.delay;
            break;
        case id_gelix_duration:
            value_data[0] = gelix_config.duration;
            break;
    }
}

void gelix_config_save(void) {
    ...
    if (*channel_id == id_custom_channel) {
        switch (*command_id) {
            case id_custom_set_value:
                if (data[2] == id_gelix_mode || data[2] == id_gelix_delay || data[2] == id_gelix_duration) {
                    gelix_config_set_value(value_id_and_data);
                } else {
            ...
            case id_custom_get_value:
                ...
                if (data[2] == id_gelix_mode || data[2] == id_gelix_delay || data[2] == id_gelix_duration) {
                    gelix_config_get_value(value_id_and_data);
                } else {
                    fader_config_get_value(value_id_and_data);
                }
                // gelix_config_get_value(value_id_and_data);
